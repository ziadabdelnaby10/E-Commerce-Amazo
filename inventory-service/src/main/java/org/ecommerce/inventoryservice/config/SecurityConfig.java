...
/**
 * Standard Spring Security resource-server configuration.
 *
 * <p>The service validates the bearer token itself (defense in depth) instead of trusting the API
 * gateway, and maps the JWT {@code roles}/{@code permissions} claims to Spring authorities so that
 * {@code @PreAuthorize} behaves exactly like in the other microservices.</p>
 */
@Configuration
@EnableWebSecurity
